...
class UpdateDialog(wx.Dialog):
    def __init__(self, parent, updater: AutoUpdater):
        ...
        title = wx.StaticText(
            panel,
            label=f"New Version Available: {updater.get_latest_version()}"
        )
        ...
        current_version = wx.StaticText(
            panel,
            label=f"Current Version: {updater.current_version}"
        )
        sizer.Add(current_version, 0, wx.LEFT | wx.RIGHT | wx.BOTTOM, 10)

        sizer.Add(wx.StaticLine(panel, style=wx.LI_HORIZONTAL), 0, wx.EXPAND | wx.ALL, 5)
        ...
        sizer.Add(self.progress_panel, 0, wx.EXPAND | wx.ALL, 10)
