...
class SemesterEnrollmentRepository:
    ...
    def get_student_program_details_by_id(self, student_program_db_id: int):
        return self._student_repo.get_student_program_details_by_id(
            student_program_db_id
        )

    def get_student_semester_by_id(self, student_semester_db_id: int):
        return self._student_repo.get_student_semester_by_id(student_semester_db_id)

    def upsert_student_semester(
        self, student_program_db_id: int, data: dict
    ) -> tuple[bool, str, Optional[int]]:
        return self._student_repo.upsert_student_semester(student_program_db_id, data)

    def get_semester_module_credits_by_cms_id(
        self, semester_module_cms_id: int
    ) -> Optional[float]:
        return self._student_repo.get_semester_module_credits_by_cms_id(
